class GomokuGame {
    ...
    loadImages() {
        ...
        imageFiles.forEach((filename, index) => {
            ...
            img.onload = () => {
                this.imagesLoaded++;
                if (this.imagesLoaded === imageFiles.length && this.backgroundImage && this.backgroundImage.complete) {
                    this.drawBoard();
                }
            };
            img.src = filename;
        ...
        this.backgroundImage.src = '666.jpg';
    }
    ...
    handleCanvasClick(e) {
        ...
        this.handlePiecePlace(coords.x, coords.y);
    }
    ...
    getCanvasCoordinates(clientX, clientY) {
        ...
        console.log(`原始坐标: (${clientX}, ${clientY}), Canvas区域: left=${rect.left}, top=${rect.top}`);
        console.log(`相对坐标: (${x}, ${y}), Canvas尺寸: ${rect.width}x${rect.height}`);
        
        return { x, y };
    }
    
    handlePiecePlace(x, y) {
        // 游戏状态检查
        if (this.gameOver) {
        ...
        console.log(`点击位置: (${x.toFixed(1)}, ${y.toFixed(1)}), 格子大小: ${this.cellSize}`);
        console.log(`计算得到格子: (${gridRow}, ${gridCol})`);
        
        // 边界和有效性检查
        if (this.isValidPlacement(gridRow, gridCol)) {
            console.log(`有效落子: 在格子(${gridRow}, ${gridCol})放置棋子`);
            this.placePiece(gridRow, gridCol);
        } else {
            console.log(`无效落子: 格子(${gridRow}, ${gridCol})超出范围或已被占用`);
        }
    }
    ...
    placePiece(row, col) {
        ...
        // 分配随机图片（固定不变）
        const imageIndex = Math.floor(Math.random() * this.pieceImages.length);
        this.pieceImageIndexes[row][col] = imageIndex;
        
        console.log(`棋子已放置: 玩家${this.currentPlayer}, 位置(${row}, ${col}), 图片索引${imageIndex}`);
    ...
    makeMove(row, col, player) {
        ...
        this.drawBoard();
        console.log(`AI落子: 玩家${player}, 位置(${row}, ${col}), 图片索引${newPieceImageIndex}`);
    }
    ...
    drawBoard() {
        ...
        if (this.backgroundImage && this.backgroundImage.complete) {
            ...
            // 添加半透明层以提高网格线可见度
            ctx.fillStyle = 'rgba(222, 184, 135, 0.4)'; // 淡棕色半透明层
        ...
        // 绘制网格线
        ctx.strokeStyle = 'rgba(139, 69, 19, 0.9)'; // 较深的线条以提高可见度
        ...
        // 绘制天元和星位（9x9棋盘）
        ctx.fillStyle = 'rgba(139, 69, 19, 0.9)'; // 更明显的星位
        const starPoints = [
            [2, 2], [2, 6], [4, 4], [6, 2], [6, 6] // 适配9x9棋盘
        ];
        
        for (const [row, col] of starPoints) {
            ctx.beginPath();
            ctx.arc(col * this.cellSize + this.cellSize/2, 
                   row * this.cellSize + this.cellSize/2, 4, 0, 2 * Math.PI); // 稍大一些的星位
            ctx.fill();
        }
