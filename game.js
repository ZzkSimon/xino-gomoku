class GomokuGame {
    ...
    processClick(x, y) {
        ...
        // 直接定位到格子中心：使用Math.floor找到点击所在的格子
        const col = Math.floor(x / this.cellSize);
        const row = Math.floor(y / this.cellSize);
        
        console.log(`点击分析: 点击位置(${x.toFixed(2)}, ${y.toFixed(2)}), 目标格子(${row}, ${col}), 格子大小: ${this.cellSize}`);
        ...
        if (row >= 0 && row < this.boardSize && 
            ...
            console.log(`点击失败: 超出棋盘范围或位置已被占用`);
        }
    }
